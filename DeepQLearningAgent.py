...
class DeepQLearningAgent:
    ...
    def choose_action(self, state):
        action, qvalues = self.sess.run([self.act, self.out], feed_dict={self.x: self.one_hot_encode(state)})
    ...
    def update(self, state, next_state, reward, action):
        qnext_values = self.sess.run([self.out], feed_dict={self.x: self.one_hot_encode(next_state)})
        targetq = self.q_predict
        targetq[0, action] = reward + self.gamma * np.max(qnext_values)
        self.sess.run([self.train_step], feed_dict={self.x: self.one_hot_encode(state), self.t: targetq})
